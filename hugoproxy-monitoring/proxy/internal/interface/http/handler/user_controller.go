...
import (
	...
	userUsecase "gitlab.com/s.izotov81/hugoproxy/internal/usecase/user"
...
func NewUserController(userService *userUsecase.UserService, responder responder.Responder) *UserController {
...
func (c *UserController) RegisterUser(w http.ResponseWriter, r *http.Request) {
	...
	userEntity, err := c.userService.GetUserByEmail(r.Context(), req.Email)
	if err != nil {
		c.responder.Error(w, http.StatusInternalServerError, fmt.Sprintf("User created but failed to retrieve: %v", err))
		return
	}

	c.responder.Respond(w, http.StatusCreated, userEntity)
}
...
func (c *UserController) GetUser(w http.ResponseWriter, r *http.Request) {
	...
	userEntity, err := c.userService.GetUser(r.Context(), id)
	if err != nil {
		status := http.StatusInternalServerError
		message := err.Error()
	...
	c.responder.Respond(w, http.StatusOK, userEntity)
}
...
func (c *UserController) GetUserByEmail(w http.ResponseWriter, r *http.Request) {
	...
	userEntity, err := c.userService.GetUserByEmail(r.Context(), email)
	if err != nil {
		status := http.StatusInternalServerError
		message := err.Error()
		if errors.Is(err, userUsecase.ErrUserNotFound) {
